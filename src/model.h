...
struct Block {
  Block(
    ...
    const Tensor* moegate,
     ...
     // bias qkv
     const Tensor* qbias=nullptr,
     const Tensor* kbias=nullptr,
     const Tensor* vbias=nullptr
  );
  ~Block();
  ...
  template <typename T>
  T* qbias() const { return static_cast<T*>(_qbias); }
  template <typename T>
  T* kbias() const { return static_cast<T*>(_kbias); }
  template <typename T>
  T* vbias() const { return static_cast<T*>(_vbias); }
...
private:
  ...
  void* _qbias = nullptr; // (dim)
  void* _kbias = nullptr; // (dim)
  void* _vbias = nullptr; // (dim)
