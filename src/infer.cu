...
template <typename T>
__global__
void fused_qkv_matmul_add_bias_clip(
  const T* wq,      // (q_dim, dim)
  const T* wk,      // (kv_dim, dim)
  const T* wv,      // (kv_dim, dim)
  const T* qbias,      // (q_dim)
  const T* kbias,      // (kv_dim)
  const T* vbias,      // (kv_dim)
  const float* x,   // (dim,)
  int dim,          // input dimension
  int q_dim,        // n_heads * head_dim
  int kv_dim,       // n_kv_heads * head_dim
  ...
  if (warp_id < q_dim) {
    ...
    w = wk + (warp_id - q_dim) * dim;
    out = k_out + (warp_id - q_dim);
    bias = kbias + (warp_id - q_dim);
  } else {
    // Computing V
    w = wv + (warp_id - q_dim - kv_dim) * dim;
    out = v_out + (warp_id - q_dim - kv_dim);
    bias = vbias + (warp_id - q_dim - kv_dim);
  }
  ...
  // Write result with clipping
  if (offset == 0) {
    row_sum = row_sum + __half2float(*bias);
    row_sum = row_sum < -qkv_clip ? -qkv_clip : (row_sum > qkv_clip ? qkv_clip : row_sum);
    *out = row_sum;
  }
}
...
#define STATIC_KERNEL(x) if (!s.graph().is_created) x;
...

  {
    ...
    else if (c.arch == "Qwen2ForCausalLM") {
      STATIC_KERNEL((fused_qkv_matmul_add_bias_clip<<<total_rows, warp_size, 0, s.stream()>>>(
        wq<T>(),
        wk<T>(),
        wv<T>(),
        qbias<T>(),
