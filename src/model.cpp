...
Block::Block(
  ...
  if (qbias) {
    // qkv bias
    _qbias = check_tensor(
      qbias, config->weight_dtype, {config->n_heads * config->head_dim, 0, 0}
    );
    _kbias = check_tensor(
      kbias, config->weight_dtype, {config->n_kv_heads * config->head_dim, 0, 0}
    );
    _vbias = check_tensor(
      vbias, config->weight_dtype, {config->n_kv_heads * config->head_dim, 0, 0}
    );
  }
...
void Block::cuda() {
  ...
  if (_qbias) {
    _qbias = upload_cuda(_qbias, _config->dim * weight_size);
    _kbias = upload_cuda(_kbias, _config->dim * weight_size);
    _vbias = upload_cuda(_vbias, _config->dim * weight_size);
  }
