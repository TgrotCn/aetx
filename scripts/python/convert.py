...
class Metadata:
  def __init__(self, config, dtype):
    ...
    self.dtype = dtype
    if arch in ["MistralForCausalLM", "LlamaForCausalLM", "MixtralForCausalLM", "Qwen2ForCausalLM"]:
  ...
  def to_dict(self):
    ...
    if self.arch in ["MistralForCausalLM", "LlamaForCausalLM", "MixtralForCausalLM", "Qwen2ForCausalLM"]:
...
def load_weights(model_files, dtype_str, metadata, tie_word_embeddings):
  ...
  def conv(t):
    nonlocal progress
    progress += 1
    # print(f"\rConverting tensor {progress}: {t.shape}")
    return t.to(dtype)
  ...
  for l in range(config["num_hidden_layers"]):
    ...
    # ptensor = permute_reverse(weights[f"model.layers.{l}.self_attn.q_proj.weight"], n_heads, rotary_dim)
    # print(n_heads, rotary_dim)
    # otensor = weights[f"model.layers.{l}.self_attn.q_proj.weight"]
    # print(torch.all(ptensor == otensor))
    
    tensors[f"model.layers.{l}.attn.wv.weight"] = conv(weights[f"model.layers.{l}.self_attn.v_proj.weight"])

    tensors[f"model.layers.{l}.attn.wq.bias"] = conv(permute_reverse(weights[f"model.layers.{l}.self_attn.q_proj.bias"], n_heads, rotary_dim))
    tensors[f"model.layers.{l}.attn.wk.bias"] = conv(permute_reverse(weights[f"model.layers.{l}.self_attn.k_proj.bias"], n_kv_heads, rotary_dim))
    tensors[f"model.layers.{l}.attn.wv.bias"] = conv(weights[f"model.layers.{l}.self_attn.v_proj.bias"])
